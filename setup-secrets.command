...
# ── Anthropic API key ────────────────────────────────
echo "Step 1 of 3: Anthropic API key"
...
echo "Step 2 of 3: Brave Search API key"
...
# ── Resend API key ───────────────────────────────────
echo "Step 3 of 3: Resend API key (for license key emails)"
echo "Get yours free at: https://resend.com  (sign up, verify influentia.io domain)"
echo ""
read -p "Paste your Resend API key (re_...): " RESEND_KEY
if [ -z "$RESEND_KEY" ]; then
  echo "⚠️  Skipped Resend key. License emails will not be sent."
else
  echo "$RESEND_KEY" | npx wrangler secret put RESEND_API_KEY --env production
  echo ""
fi
