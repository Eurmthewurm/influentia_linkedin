...
PAUSE_DATA=$(curl -s "$BASE_URL/api/linkedin/status" 2>/dev/null)
IS_PAUSED=$(echo "$PAUSE_DATA" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('pause',{}).get('paused', False))" 2>/dev/null)
IS_MANUAL=$(echo "$PAUSE_DATA"  | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('pause',{}).get('manual', False))" 2>/dev/null)

if [ "$IS_PAUSED" = "True" ] && [ "$IS_MANUAL" != "True" ]; then
    log "⏸  LinkedIn is paused (security challenge). Skipping today's run entirely."
    log "   Resolve the challenge in your browser, then resume from the dashboard."
    exit 0
fi

# ── Step 3: Run automation sequence ──────────────────────────────────────────
log "═══════════════════════════════════════════════"
if [ "$IS_PAUSED" = "True" ]; then
    log "Starting daily LinkedIn automation sequence (MANUALLY PAUSED — replies only)"
else
...
if [ "$IS_PAUSED" != "True" ]; then
    ...
    log "⏸  Outreach paused by user — skipping find/connect/send/followup."
    log "   Still checking replies so active conversations stay warm."
fi
