...
def _extract_prospect_data(prospect: dict, profile_data: dict, posts: list) -> dict:
    ...
    # Always prefer the live-scraped name from the profile page over the stored
    # lead name — the stored name can be wrong if the source data had a mismatch.
    live_first = (pd.get("firstName") or "").strip()
    live_last  = (pd.get("lastName") or "").strip()
    live_full  = f"{live_first} {live_last}".strip()
    prospect_name = live_full or prospect.get("name", "")
...
- NO sycophantic openers: "Absolutely", "Certainly", "Great question", "That's a great point", "Great insight", "Totally", "100%", "For sure"
- NO affirmations that add nothing: "This resonates", "Couldn't agree more", "Love this", "So true", "I totally get that", "Makes complete sense"
- NO formal filler: "I hope this finds you well", "Feel free to reach out", "Don't hesitate to", "Please don't hesitate", "I wanted to reach out"
- NO hollow intensifiers: "particularly compelling", "genuinely rare", "incredibly important", "truly fascinating", "deeply insightful"
- NO AI sentence frames: "what makes this X is", "the tension between X and Y", "the reality is", "here's the truth", "what nobody tells you"
- NO dramatic openers: "In a world where…", "Most people X. The few who win Y.", "Stop doing X. Start doing Y."
- NO hashtags
- NO buzzwords (these scream AI): delve, realm, harness, unlock, tapestry, paradigm, cutting-edge, revolutionize, landscape, pivotal, crucial, meticulously, vibrant, unparalleled, underscore, leverage, synergy, innovative, game-changer, testament, meticulous, highlight, emphasize, groundbreaking, foster, enhance, holistic, garner, pioneering, transformative, seamless, optimize, robust, breakthrough, empower, streamline, elevate, adaptive, visionary, disruptive, reimagine, scalable, effortless, data-driven, proactive, unprecedented
- NO "It's worth noting", "It goes without saying", "At the end of the day", "In today's landscape", "It's never been easier", "More than ever"
...
def generate_follow_up(prospect: dict, days_since: int, first_msg_text: str, follow_up_number: int = 1) -> str:
    """Generate a follow-up message for a prospect who hasn't replied."""
    prompt_file = "follow_up_2.txt" if follow_up_number >= 2 else "follow_up.txt"
    template = _load_prompt(prompt_file)
    template = _fill_config_vars(template)
    # Use first name only for follow-ups — friendlier and avoids surname issues
    stored_name = prospect.get("name", "")
    first_name  = stored_name.split()[0] if stored_name.strip() else ""
    template = template.replace("{prospect_name}", first_name)
