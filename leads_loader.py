...
def load_leads_from_excel() -> list:
    ...
    Returns empty list if no path is configured or file doesn't exist.
    """
    import os
    if not LEADS_EXCEL_PATH or not LEADS_EXCEL_PATH.strip():
        return []  # No Excel file configured — fine, leads come from ICP search instead
    if not os.path.exists(LEADS_EXCEL_PATH):
        print(f"[leads_loader] Excel file not found: {LEADS_EXCEL_PATH!r} — skipping")
        return []
    ...
    for row in ws.iter_rows(min_row=HEADER_ROW + 1, values_only=True):
        # Pad short rows (merged divider cells return fewer columns)
        padded = list(row) + [None] * (12 - len(row))
        num, name, title, company, industry, email, phone, linkedin, website, city, state, country = padded[:12]
