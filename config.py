...
#
# TWO PROFILES BELOW — comment/uncomment to switch:
#   A) Authentik Studio (video content agency)
#   B) Influentia (outreach automation tool)  ← uncomment when using "Influentia Buyers" ICP
...
# ── B) Influentia ─────────────────────────────────────────────────────────────
# Uncomment the block below (and comment out the Authentik block above)
# when running outreach for the "Influentia Buyers" ICP.
#
# YOUR_OFFERING = """
# # Influentia — LinkedIn + Reddit Outreach Autopilot for B2B Founders
#
# Influentia is a desktop app (Mac, Windows, Linux) that automates B2B outreach
# across LinkedIn and Reddit. It runs on the customer's own machine — not a shared
# cloud server — so LinkedIn cannot detect it as automation.
#
# ## The problem we solve
#
# B2B founders, agency owners, and consultants doing their own sales face two bad
# options: spend hours a day on manual outreach, or use a cloud-IP tool (Expandi,
# Phantombuster, Lemlist) that gets their account restricted. There is no middle
# ground that is both automated and safe — until now.
#
# ## What Influentia does
#
# Describe your ideal customer once. Influentia finds matching leads via web search
# (no Sales Navigator needed), visits their LinkedIn, reads their recent posts, and
# writes a personalised message in your voice. Follows up automatically. Flags replies
# that look like they are ready to book. Also scans Reddit daily for posts expressing
# buying intent and drafts genuine, helpful replies for one-click approval.
#
# ## Who we serve
#
# B2B SaaS founders ($1k-$10k MRR), agency owners, and consultants doing their own
# sales. Cannot justify a full-time SDR. Want to spend 10 minutes a day on outreach,
# not 2 hours.
#
# ## Pricing
#
# $97/month flat. No per-lead or per-message fees. 14-day money-back guarantee.
# AI (Claude) and web search are bundled — no API keys needed.
# """
...
DELAY_BETWEEN_REQUESTS_SECONDS  = _env_int("DELAY_BETWEEN_REQUESTS_SECONDS", 60)
...
MAX_FOLLOW_UPS       = _env_int("MAX_FOLLOW_UPS", 2)
...
def validate_config(strict: bool = False) -> list:
    ...
    # AI and search are now handled by the Influentia proxy — no local API keys needed.
