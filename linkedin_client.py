...
class LinkedInClient:
    ...
    def get_profile_posts(self, linkedin_url: str, count: int = 3) -> list:
        """
        Visit the profile's recent-activity page and grab post text snippets.
        Uses multiple selector fallbacks to handle LinkedIn's ever-changing UI.
        """
        public_id = self._extract_public_id(linkedin_url)
        try:
            ...
            posts = self._page.evaluate(f"""() => {{
                ...
                // Selector groups tried in order — LinkedIn changes class names often.
                // Each group is [container_selector, text_selector_within_container].
                // If container_selector is null, text_selector finds text nodes directly.
                const strategies = [
                    // 2024-2025 LinkedIn UI
                    ...
                    // Newer card-based layout
                    [null, '.update-components-text__text-view'],
                    [null, '[data-test-id="main-feed-activity-card"] .break-words'],
                    // Generic fallback — any substantial text block inside an activity card
                    [null, '.fie-impression-container .break-words'],
                ];

                for (const [containerSel, textSel] of strategies) {{
                    if (results.length >= n) break;
                    let candidates;
                    if (containerSel) {{
                        const containers = [...document.querySelectorAll(containerSel)];
                        candidates = containers
                            .map(c => c.querySelector(textSel))
                            .filter(Boolean);
                    }} else {{
                        candidates = [...document.querySelectorAll(textSel)];
                    }}
                    for (const el of candidates) {{
                        if (results.length >= n) break;
                        const text = (el.innerText || el.textContent || '').trim();
                        if (text.length > 30) {{
                            results.push({{ commentary: {{ text: text.slice(0, 300) }} }});
                        }}
                    }}
                }}
                return results;
            ...
            self._tick()
            if posts:
                log.debug(f"Fetched {len(posts)} posts for {public_id}")
            return posts or []
        except StopSignal:
            raise
        except Exception as e:
    ...
    def get_my_connections(self, limit: int = 300) -> set:
        ...
        try:
            ...
            prev_link_count = 0  # track raw link count to detect page growth
            ...
            for scroll_attempt in range(40):  # up to ~400 connections
                ...
                if len(new_ids) == 0:
                    stale_scrolls += 1
                    if stale_scrolls >= 5:
                        # Five scrolls in a row with no new IDs — we've hit the end
                        break
                else:
                    stale_scrolls = 0

                # LinkedIn's connections list uses a virtual scroll that requires
                # both JS scrollTo AND mouse wheel events to trigger lazy loading.
                # Keyboard End alone is not enough — fire all three.
                self._safe_evaluate("""() => {
                    window.scrollTo(0, document.body.scrollHeight);
                    const containers = document.querySelectorAll(
                        'div[class*="scaffold-finite-scroll"], div[class*="connections"], ' +
                        'div[class*="mn-connections"], ul[class*="mn-connections"]'
                    );
                    for (const c of containers) {
                        c.scrollTop = c.scrollHeight;
                    }
                }""")
                # Mouse wheel scroll — LinkedIn's virtual list often only responds to this
                self._page.mouse.wheel(0, 800)
                self._page.keyboard.press("End")
                # Give the network request + render time — LinkedIn lazy-loads in batches
                time.sleep(random.uniform(2.5, 4.0))

                # Wait for new /in/ links to appear (up to 6s)
                current_link_count = self._safe_evaluate(
                    "() => document.querySelectorAll('a[href*=\"/in/\"]').length"
                ) or 0
                if current_link_count > prev_link_count:
                    prev_link_count = current_link_count
                    stale_scrolls = 0  # reset stale counter when page grew
                else:
                    try:
                        self._page.wait_for_function(
                            f"() => document.querySelectorAll('a[href*=\"/in/\"]').length > {current_link_count}",
                            timeout=6000,
                        )
                        stale_scrolls = 0
                    except Exception:
                        pass  # no new links — stale counter increments normally
    ...
    def send_connection_request(self, linkedin_url: str, message: str = "") -> bool:
        ...
        If message is provided, clicks "Add a note" and types it (≤280 chars).
        Otherwise clicks "Send without a note".
        """
        public_id = self._extract_public_id(linkedin_url)
        if not public_id:
            return False
        try:
            ...
            if clicked:
                ...
                if note:
                    # Try clicking "Add a note" first
                    added_note = self._page.evaluate("""() => {
                        const buttons = document.querySelectorAll('button');
                        for (const btn of buttons) {
                            const text = btn.innerText.trim().toLowerCase();
                            if (text === 'add a note' || text.includes('add a note')) {
                                btn.click();
                                return true;
                            }
                        }
                    ...
                    if added_note:
                        ...
                        try:
                            textarea = self._page.query_selector('textarea[name="message"], textarea#custom-message, textarea')
                            if textarea:
                                textarea.click()
                                textarea.fill(note)
                                time.sleep(0.5)
                        ...
                        self._page.evaluate("""() => {
                            ...
                            for (const btn of buttons) {
                                const text = btn.innerText.trim().toLowerCase();
                                const ariaLabel = (btn.getAttribute('aria-label') || '').toLowerCase();
                                if (text === 'send' || ariaLabel.includes('send invitation')) {
                                    btn.click();
                                    return true;
                                }
                            }
                        ...
                        # Modal didn't have "Add a note" — just send without
                        self._page.evaluate("""() => {
                            const buttons = document.querySelectorAll('button');
                            for (const btn of buttons) {
                                const text = btn.innerText.trim().toLowerCase();
                    ...
                    # No note requested — send without
                    self._page.evaluate("""() => {
                        const buttons = document.querySelectorAll('button');
                        for (const btn of buttons) {
                            const text = btn.innerText.trim().toLowerCase();
                ...
                time.sleep(1)
                log.info(f"Connection request sent → {public_id}" + (f" (with note)" if note else ""))
                self._tick()
                return True
            else:
    ...
    def get_conversation(self, linkedin_url: str) -> list:
        ...
        try:
            ...
            # Walk message GROUPS so every bubble in a group inherits the same sender.
            # PRIMARY signal: LinkedIn marks incoming groups with the CSS class
            # 'msg-s-message-group--incoming'. This is structural markup, far more
            # reliable than parsing the sender's name text.
            # FALLBACK: if the class is absent, use sender name matching.
            # LAST RESORT: unknown sender → "me" (safer than "them" — we'd rather
            # miss a reply than send a duplicate message to someone).
            messages = self._page.evaluate("""() => {
                ...
                groups.forEach(group => {
                    // LinkedIn marks received-message groups with this class.
                    // Absence = messages we sent.
                    const isIncoming = group.classList.contains('msg-s-message-group--incoming');
                    ...
                    bubbles.forEach(bubble => {
                        ...
                        if (text) {
                            msgs.push({ text, sender_name: senderName, is_incoming: isIncoming });
                        }
                    });
                });
                // Fallback: flat list if groups approach returns nothing
                if (!msgs.length) {
            ...
            for m in (messages or []):
                is_incoming = m.get("is_incoming")
                sn = m.get("sender_name", "").lower().strip()

                if is_incoming is True:
                    # LinkedIn's own markup says this is an incoming message — trust it.
                    sender = "them"
                elif is_incoming is False:
                    # LinkedIn's own markup says this is outgoing — we sent it.
                    sender = "me"
                else:
                    # Flat-list fallback: no is_incoming signal available.
                    # Unknown sender → "me". Always safer to miss a reply
                    # than to echo our own message back as a fake prospect reply.
                    if not sn:
                        sender = "me"
