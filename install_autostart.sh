...
# Sets up the Influentia server to start automatically at login on macOS.
# Run once from inside the influentia folder:
#   bash install_autostart.sh
# ─────────────────────────────────────────────────────────────────────────────

# Get the actual folder this script lives in (works regardless of where user put it)
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LABEL="com.influentia.server"
PLIST_DEST="$HOME/Library/LaunchAgents/$LABEL.plist"
PYTHON_PATH="$SCRIPT_DIR/venv/bin/python3"
SERVER_PATH="$SCRIPT_DIR/server.py"

echo "════════════════════════════════════════════════════════════"
echo "  Influentia — Auto-Start Installer"
echo "════════════════════════════════════════════════════════════"
echo ""
...
if [ ! -f "$SERVER_PATH" ]; then
    ...
    echo "   Make sure you're running this from inside the influentia folder."
    exit 1
fi
...
echo "✓ Found Influentia at: $SCRIPT_DIR"
echo ""
